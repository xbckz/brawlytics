...
def load_matches_data():
    ...
    try:
        ...
        if 'battle_time' in df.columns:
            # Check if battle_time is a string or already a datetime
            if df['battle_time'].dtype == 'object':
                # It's a string, parse it
                df['battle_time'] = pd.to_datetime(df['battle_time'], format='%Y%m%dT%H%M%S.%fZ', utc=True)
            else:
                # It's already a datetime, just localize to UTC
                df['battle_time'] = pd.to_datetime(df['battle_time']).dt.tz_localize('UTC')
        ...
        matches_df = df
        print(f"DEBUG: battle_time type after load: {type(matches_df['battle_time'].iloc[0])}")
...
def calculate_all_stats_offseason():
    ...
    if matches_df is not None and not matches_df.empty and 'battle_time' in matches_df.columns:
        print(f"DEBUG in calculate_all_stats_offseason: battle_time type = {type(matches_df['battle_time'].iloc[0])}")
        print(f"DEBUG: First battle_time value = {matches_df['battle_time'].iloc[0]}")
        print(f"DEBUG: Last battle_time value = {matches_df['battle_time'].max()}")
    ...
    for match_idx, match in relevant_matches:
        ...
        if pd.notna(battle_time):
            if isinstance(battle_time, str):
                battle_time = pd.to_datetime(battle_time, format='%Y%m%dT%H%M%S.%fZ', utc=True)
            time_rounded = battle_time.floor('30min')
        else:
...
class WelcomeView(View):
    ...
    async def meta_button(self, interaction: discord.Interaction, button: Button):
        ...
        # Calculate last update time - FIXED VERSION
        last_update = "Unknown"
        if matches_df is not None and not matches_df.empty and 'battle_time' in matches_df.columns:
            try:
                ...
                if pd.notna(latest_match):
                    # Ensure it's a Timestamp and timezone-aware
                    if not isinstance(latest_match, pd.Timestamp):
                        latest_match = pd.Timestamp(latest_match)
                    
                    # Make timezone-aware if needed
                    if latest_match.tz is None:
                        latest_match = latest_match.tz_localize('UTC')
                    else:
                        latest_match = latest_match.tz_convert('UTC')
                    
                    # Get current time in UTC
                    now_utc = pd.Timestamp.now(tz='UTC')
                    
                    # Calculate difference
                    time_diff = now_utc - latest_match
                    total_seconds = time_diff.total_seconds()
                    
                    # Handle negative values (future dates - data error)
                    if total_seconds < 0:
                ...
                print(f"Error calculating last update time: {e}")
                last_update = "Unknown"
...
class WelcomeViewOffseason(View):
    ...
    async def meta_button(self, interaction: discord.Interaction, button: Button):
        ...
        if matches_df is not None and not matches_df.empty and 'battle_time' in matches_df.columns:
            try:
                # Get the latest match time (already a proper UTC timestamp)
                latest_match = matches_df['battle_time'].max()
                
                if pd.notna(latest_match):
                    # Use Discord's raw timestamp - it already has timezone info
                    now_utc = pd.Timestamp(interaction.created_at)
                    ...
                    print(f"Discord created_at RAW: {interaction.created_at}")
                    print(f"Discord created_at timezone: {interaction.created_at.tzinfo}")
                    print(f"Now (after conversion): {now_utc}")
                    print(f"Diff: {total_seconds}s, Display: {last_update}")
